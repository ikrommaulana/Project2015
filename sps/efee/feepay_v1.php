...
if (($op==save)&&((count($cbox)>0)||($ontebus>0)||($jumbayar>0))) {
			...
			if($isexist>0){
					...
					PERHATIAN!<BR>
					Sila klik untuk lihat resit.<BR>
					Sila SEMAK resit dan PASTIKAN transaksi betul.<BR>
					Jika transaksi resit tidak betul, sila ulangi proses pembayaran semula. TQ
					</a>";
...
<table width="100%" cellspacing="0">
       <tr>
		...
		<td id="mytabletitle" width="10%"><?php echo strtoupper($lg_receipt);?></td>
...
<?php 
	...
	$sql="select feeset.*,type.etc from feeset INNER JOIN type ON feeset.name=type.prm where feeset.sch_id=$sid and year='$year' and type.grp='yuran' and type.val=$feetype";
	$resfee=mysql_query($sql)or die("$sql - query failed:".mysql_error());
	$xxfeetype=$feetype;
	while($rowfee=mysql_fetch_assoc($resfee)){
