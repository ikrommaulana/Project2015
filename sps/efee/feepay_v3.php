...
110218 - repair tebus
110513 - add sessionid	
111017 - update fee code for feepay and feestu
120906 - change $sch_id to $sid 
**/

$vdate="120906";//new - update feestu
$vmod="v6.1.1";
...
if (($op==save)&&((count($cbox)>0)||($ontebus>0)||($jumbayar>0))) {
			//SEMAK DULU DAH ADA RECORD PAYMENT KE.. ELAKKAN DUPLICATE
			$sql="select * from feetrans where sessionid='$sessionid' and year='$year' limit 1";
			$res=mysql_query($sql)or die("query failed:".mysql_error());
			$isexist=mysql_num_rows($res);
			if($isexist>0){
					$row=mysql_fetch_assoc($res);
					$feeid=$row['id'];
					echo "<a href=\"$FN_FEERESIT.php?id=$feeid\"> 
					PERHATIAN!<BR>
					Sila klik untuk lihat resi.<BR>
					Sila SEMAK resi dan PASTIKAN transaksi betul.<BR>
					Jika transaksi resi tidak betul, sila ulangi proses pembayaran semula. TQ
					</a>";
			...
			while($rowfeetype=mysql_fetch_assoc($resfeetype)){
					...
					while($rowfee=mysql_fetch_assoc($resfee)){
							...
							if($feeval==-1){
										...
										$paydt="";
										$xresitno="";
										$sql="select feepay.*,feetrans.paydate from feepay INNER JOIN feetrans ON feepay.tid=feetrans.id where feepay.stu_uid='$uid' and 
												feepay.year='$year' and feepay.fee='$feename' and feepay.isdelete=0";
										$res2=mysql_query($sql) or die("$sql - query failed:".mysql_error());
										if($row2=mysql_fetch_assoc($res2)){
											$feeval=$row2['rm'];
											$resit=$row2['tid'];
							...
							if(!$row_feestux=mysql_fetch_assoc($res_feestux)){
								$sql="insert into feestu(dtm,sid,uid,ses,fee,typ,val,sta,des,rno,resitno,pdt,adm,mon,cod)values(now(),$sid,'$uid','$year',
								'$feename',$feetype,$feeval,$feesta,'$fdes','$resit','$xresitno','$paydt','$username','$feemonth','$feecode')";
								$res=mysql_query($sql) or die("$sql - query failed:".mysql_error());
							}
					}
			}
			...
			$sql="insert into feetrans (cdate,year,sch_id,stu_id,stu_uid,rm,admin,paytype,nocek,paydate,rujukan,resitno,name,ic,sessionid)
			values(now(),'$year',$sid,$stu_id,'$stu_uid',$jumbayar,'$username','$paytype','$nocek','$paydate','$rujukan','$newresitno','$namex','$ic','$sessionid')";
			...
			if (count($cbox)>0) {
					for ($i=0; $i<count($cbox); $i++) {
						$data=$cbox[$i];
						list($prm,$val,$xxfeetype,$xxfeecode)=explode("|",$data);
						$sql="insert into feepay (cdate,year,sch_id,stu_id,stu_uid,rm,fee,resitno,type,tid,admin,item_type,item_code)
							values(now(),'$year',$sid,$stu_id,'$stu_uid',$val,'$prm','$newresitno','fee',$feeid,'$username','$xxfeetype','$xxfeecode')";
						mysql_query($sql)or die("$sql - failed:".mysql_error());
		
						$sql="update feestu set sta=1,val=$val,rno=$feeid,pdt='$paydate',adm='$username',resitno='$newresitno' where uid='$uid' and ses='$year' and sid='$sid' 
							and fee='$prm'";
						mysql_query($sql)or die("$sql - failed:".mysql_error());
					}
			}
			...
			for ($i=0; $i<count($saleunit); $i++) {
					if($saleunit[$i]>0){
						...
						$sql="insert into feepay (cdate,year,sch_id,stu_id,stu_uid,rm,fee,resitno,
								...
								values(now(),'$year',$sid,$stu_id,'$stu_uid',$tot,'$nam','$newresitno',
			...
			if($onadvance){
				$sql="insert into feepay (cdate,year,sch_id,stu_id,stu_uid,rm,fee,tid,resitno,type,etc,admin)
				values(now(),'$year',$sid,$stu_id,'$stu_uid',$payadvance,'ADVANCE',$feeid,'$newresitno','fee','$advdes','$username')";
				if(!mysql_query($sql))
							 mysql_query($sql)or die("$sql - failed:".mysql_error());
			}
			if($lainlainval>0){
				$sql="insert into feepay (cdate,year,sch_id,stu_id,stu_uid,rm,fee,etc,tid,resitno,type,admin)
				values(now(),'$year',$sid,$stu_id,'$stu_uid',$lainlainval,'Lain-Lain','$lainlaindes',$feeid,'$newresitno','fee','$username')";
				if(!mysql_query($sql))
							mysql_query($sql)or die("$sql - failed:".mysql_error());
			}
			if($jumtebus>0){
				$jumteb="-$jumtebus";
				$sql="insert into feepay (cdate,year,sch_id,stu_id,stu_uid,rm,fee,etc,tid,resitno,type,admin)
				values(now(),'$year',$sid,$stu_id,'$stu_uid',$jumteb,'RECLAIM','$advdes',$feeid,'$newresitno','fee','$username')";
				if(!mysql_query($sql))
							mysql_query($sql)or die("$sql - failed:".mysql_error());
			}
			echo "<script language=\"javascript\">location.href='$FN_FEERESIT.php?id=$feeid'</script>";
...
function process_form(){
	...
	if(document.form1.onadvance.checked){
		...
		**/
		...
		ret = confirm("Iuran      Rp "+total_fee+"\r\nJualan Lain     Rp "+total_jualan+"\r\nLain-Lain Rp "+total_lain+ "\r\nAdvance Rp "+advance+"\r\nKekurangan Biaya     Rp "+total_tebus+ "\r\nJUMLAH PEMBAYARAN Rp "+total_pay+ ". Konfirmasikan?");
		if (ret == true){
...
<form name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
	...
	<input type="hidden" name="sessionid" value="<?php echo "$username".time();?>">
...
<a href="#" onClick="javascript:href='<?php echo $FN_FEECFG;?>.php?uid=<?php echo "$uid";?>&sid=<?php echo "$sid";?>&year=<?php echo "$year";?>'" id="mymenuitem"><img src="../img/tool.png"><br>Configurasi</a>
...
<?php
            echo "<option value=$year>$lg_year_session $year</option>";
			$sql="select * from type where grp='session' and prm!='$year' and prm>='$reg_year' order by val desc";
            $res=mysql_query($sql)or die("query failed:".mysql_error());
            while($row=mysql_fetch_assoc($res)){
                        $s=$row['prm'];
						$v=$row['val'];
                        echo "<option value=\"$s\">$lg_year_session $s</option>";
            }
            mysql_free_result($res);					  
?>
      </select>
	  	...
	  	<tr>
		<td id="myborder"><?php echo "$lg_sum_fee";?></td>
...
<table width="100%" cellspacing="0" id="mytitlebg"> 
	  ...
	  <tr>
		 <td id="myborder" colspan="2">
			...
			&nbsp;&nbsp;&nbsp;<input name="paytype" type="radio" value="Transfer" > <?php echo strtoupper($lg_transfer);?>
	   ...
	   <tr >
		 ...
		 <td id="myborder"><input type="button" name="btnpayment" value="<?php echo "KONFIRMASI";?>" onClick="process_form()"></td>
       </tr>  
	</table>
</td>
</tr>
...
<table width="100%" cellspacing="0">
       <tr>
	   	<td id="mytabletitle" width="5%"><?php echo strtoupper($lg_status);?></td>
		<td id="mytabletitle" width="10%"><?php echo strtoupper($lg_date);?></td>
		<td id="mytabletitle" width="10%"><?php echo strtoupper($lg_no_resi);?></td>
		<td id="mytabletitle" width="5%" align="right"><?php echo strtoupper($lg_total);?>(<?php echo "Rp";?>)&nbsp;&nbsp;</td>
        <td id="mytabletitle" width="70%">&nbsp;<?php echo strtoupper($feegroup);?></td>    
       </tr>

<?php 
	
	$sql="select feeset.*,type.etc from feeset INNER JOIN type ON feeset.name=type.prm where feeset.sch_id=$sid and year='$year' and type.grp='yuran' and type.val=$feetype order by type.idx";
	$resfee=mysql_query($sql)or die("$sql - query failed:".mysql_error());
	$xxfeetype=$feetype;
	while($rowfee=mysql_fetch_assoc($resfee)){
		$xxfeecode=$rowfee['etc'];
		include($CONFIG['FILE_FORMULA']['etc']);
		if($feeval==-1)
			continue;
			...
			if($found>0){
				...
				if($feeval==0){
					...
					if(is_verify("ADMIN|KEUANGAN"))
           			echo "<input name=\"cbox[]\" type=\"checkbox\" id=\"cb\" value=\"$feename|$feeval|$xxfeetype|$xxfeecode\" onClick=\"checkx()\">";
				}
			}
		   ?>
         </td>
