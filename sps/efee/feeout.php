...
<div id="mytabletitle" style="padding:5px 5px 5px 5px "  class="printhidden">
		...
		<select name="month">
                <?php
							...
							$monthname=$row['name']; 
							$v=$row['no'];
							echo "<option value=$v>Sampai $monthname</option>";
							...
							while($row=mysql_fetch_assoc($res)){
								$p=$row['name'];
								$v=$row['no'];
								echo "<option value=$v>Sampai $p</option>";
							}
				?>
		</select>
...
<tr><td width="50%">
               ...
               <br><br>
				...
				if($maxout=="")
					...
					echo "<option value=\"$maxout\">$lg_more_then Rp $maxout</option>";
				$sql="select * from type where grp='fee_outlevel' and  prm!='$maxout' order by idx";
				$res=mysql_query($sql)or die("query failed:".mysql_error());
				while($row=mysql_fetch_assoc($res)){
					$p=$row['prm'];
					echo "<option value=$p>$lg_more_then Rp $p</option>";
				}
				if($maxout!="")
					echo "<option value=\"\">$lg_all $lg_amount</option>";
...
?>
<br>
<?php echo strtoupper("sampai");?> <?php echo strtoupper("$monthname $year");?>
</td>
<td align="right">
<?php echo strtoupper("$lg_cutoff_date");?> : <?php if($cutoff=="")echo date('Y-m-d'); else echo " $cutoff";?>
<br>Amount (Rp) : <?php if($maxout>0) echo strtoupper("$lg_more_then Rp $maxout"); else echo $lg_any_amount;?>
...
?>
	<tr>
		...
		<td class="mytableheader" style="border-right:none;" width="10%" align="right"><?php echo $lg_amount;?> (<?php echo "Rp";?>)</td>
...
<tr>
	...
	<td class="mytableheader" width="5%" align="center"><?php echo "Jumlah (Rp) ";?></td>
  </tr>
  <tr>
 <?php
