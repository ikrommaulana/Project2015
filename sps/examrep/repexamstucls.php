...
<a href="#" onClick="newwindowww('../exam/slip_exam.php')" id="mymenuitem"><img src="../img/letters.png"><br><?php echo $lg_slip;?></a>
	<div id="mymenu_space">&nbsp;&nbsp;</div>
	<div id="mymenu_seperator"></div>
	<div id="mymenu_space">&nbsp;&nbsp;</div>
...
?>
      ...
      <select name="cons" id="cons" onchange="clear_newwin();document.myform.submit();">
        <?php	
			...
			$sql="select * from type where grp='subtype' order by idx";
            		$res=mysql_query($sql)or die("$sql-query failed:".mysql_error());
           		 	while($row=mysql_fetch_assoc($res)){
                        $b=$row['prm'];
...
<?php } ?>
	<td id="mytabletitle" width="2%" align="center"><a href="#" onClick="clear_newwin();document.myform.sortranking.value=1;process_sort('examrank.total_point <?php echo "$nextdirection";?>','<?php echo "$nextdirection";?>')" title="sort">Jumlah Nilai</a></td>
<?php if($subject_type!="1"){?>
	<td id="mytabletitle" width="2%" align="center"><a href="#" onClick="clear_newwin();document.myform.sortranking.value=1;process_sort('examrank.avg <?php echo "$nextdirection";?>','<?php echo "$nextdirection";?>')" title="sort">Nilai Rata2</a></td>
...
if($clscode!=""){
	...
	$sql="select count(*) from stu,examrank where stu.sch_id=$sid and stu.uid=examrank.stu_uid  and (stu.status=6 or stu.status=99 ) and examrank.year='$year' and examrank.cls_code='$clscode' and examrank.exam='$exam'";
	$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
  	$row=mysql_fetch_row($res);
	$foundrank=$row[0];

	if(($sortranking==1)&&($foundrank>0)){
		$sql="select stu.sex,stu.name,stu.uid from stu,ses_stu,examrank where stu.sch_id=$sid and stu.uid=ses_stu.stu_uid and stu.uid=examrank.stu_uid  and (stu.status=6 or stu.status=99 ) and ses_stu.year='$year' and examrank.year='$year' and ses_stu.cls_code='$clscode' and examrank.exam='$exam' $sqlsort";
		...
		$sql="select stu.sex,stu.name,stu.uid from stu INNER JOIN ses_stu  ON stu.uid=ses_stu.stu_uid where ses_stu.sch_id=$sid and (stu.status=6 or stu.status=99) and year='$year' and ses_stu.cls_code='$clscode' $sqlsort";
	}
	//echo $sql;
	$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
  	while($row=mysql_fetch_assoc($res)){
...
?>
  <tr bgcolor="<?php echo $bg;?>" style="cursor:default" onMouseOver="this.bgColor='#CCCCFF';" onMouseOut="this.bgColor='<?php echo $bg?>';" >
	...
	<td id="myborder" ><?php echo "<a href=\"#\" title=\"Report Card\" onClick=\"newwindow('../exam/slip_exam.php?uid=$uid&year=$year&clscode=$clscode&exam=$exam&sid=$sid',0)\">$name</a>";?></td>

	 <td id="myborder" align="center">
                <?php
                        $sql="select * from exam_stu_summary where uid='$uid' and exam='$exam' and sid=$sid and year='$year' and grp='dinas'";
                        ...
                        if($mm!='')
                                ...
                                $img="../img/edit12.png";
                ?>
                <a href="../examrep/repexamcomment.php?<?php echo "clscode=$clscode&year=$year&exam=$exam&sid=$sid&grp=dinas";?>" id="pos_bottom<?php echo $q;?>" target="_blank"
	...
	<?php
				...
				while($row2=mysql_fetch_assoc($res2)){
					...
					$kk++;
					$totalmark[$kk]=$totalmark[$kk]+$point;
if($xgradingtype==0){
?>
		<td id="myborder" colspan="2" align="center" style="border-right:none">	<?php if($subtype==0){ if(($gred=="TT")||($gred=="")) echo "-"; else echo "$point";	}?>	</td>
...
?>
<td id="myborder" align="center"><?php if($subtype=="0") echo $jum_markah; else echo $jumskor;?></td>
<?php if($subject_type!="1"){ //here?>
...
<tr>
    <td id="mytabletitle" width="1%" align="center" class="printhidden"></td>
    <td id="mytabletitle" align="center"></td>
	<td id="mytabletitle" align="center"></td>
    <td id="mytabletitle" align="center"></td>
    <td id="mytabletitle" align="center"></td>
    <td id="mytabletitle" align="center">Seluruhnya</td>
...
?>
		<td id="mytabletitle" width="2%" colspan="2" align="center"><?php echo $totalmark[$kk];?></a>
		</td>
<?php } ?>
  ...
  <tr>
    ...
    <td id="mytabletitle" align="center"></td>
    <td id="mytabletitle" align="center"></td>
    <td id="mytabletitle" align="center">Rata-Rata</td>
...
?>
		<td id="mytabletitle" width="2%" colspan="2" align="center"><?php echo round($totalmark[$kk]/$q,2);?></a>
		</td>
<?php } ?>
	<td id="mytabletitle" width="2%" align="center"></td>
	<td id="mytabletitle" width="2%" align="center"></td>
...
<tr>
    ...
    <td id="mytabletitle" align="center"></td>
    <td id="mytabletitle" align="center"></td>
    <td id="mytabletitle" align="center">KKM</td>
...
?>
		<td id="mytabletitle" width="2%" colspan="2" align="center"><?php echo "$shortname";?></a>
		</td>
<?php } ?>
	<td id="mytabletitle" width="2%" align="center"></td>
	<td id="mytabletitle" width="2%" align="center"></td>
