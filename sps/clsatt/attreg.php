...
if($op=="update"){
	$uid=$_POST['uid'];
	$des=$_POST['des'];
	$sql="delete from attcls where sid=$sid and dt='$dt' and cls='$cls' and sub='$sub'";
	$res=mysql_query($sql)or die("query failed:".mysql_error());
	for ($i=0; $i<count($uid); $i++) {
		...
		if($xsta=="") 
			...
			$xsta=0;
		$xdes=$des[$i];
		$sql="insert into attcls(year,dt,sid,cls,sub,stu,sta,des,tea) values ('$y','$dt',$sid,'$cls','$sub','$xuid','$xsta','$xdes','$adm')";
        $res=mysql_query($sql)or die("query failed:".mysql_error());
	}
	$f="<font color=blue>&lt;SUCCESSFULLY UPDATED&gt</font>";
}elseif($op=="delete"){
		$sql="delete from attcls where year='$y' and dt='$dt' and sid=$sid and cls='$cls' and sub='$sub'";
        $res=mysql_query($sql)or die("query failed:".mysql_error());
...
<table width="100%" cellpadding="2" cellspacing="0" style="font-size:11px;">
  <tr>
        ...
        <td id="mytabletitle">
        <?php 	if($LG=="BM")
                    echo "Tandakan pelajar \"TIDAK HADIR\" sahaja, dan masukkan sebab jika ada";
                else
