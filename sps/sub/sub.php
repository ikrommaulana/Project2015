...
if($op!=""){
	...
	elseif($op=="update"){
		if($subid!=""){
			$sql="update sub set name='$sub',level='$addclslevel',
				sch_id='$sid',code='$cod',grp='$grp',grptype='$gtype',
				grading='$grade',sname='$sname',credit=$credit,adm='$username',ts=now(),idx='$index' where id='$subid'";
			mysql_query($sql)or die("query failed:".mysql_error());
			$f="<font color=blue>&lt;Successfully Updated&gt;</font>";
			$subid="";
		}else{
			...
			$sql="insert into sub(name,level,sch_id,code,grp,grptype,grading,sname,credit,adm,ts,idx) values ('$sub','$addclslevel',$sid,'$cod','$grp',$gtype,'$grade','$sname',$credit,'$username',now(),'$index')";
			mysql_query($sql)or die("query failed:".mysql_error());
			$id=mysql_insert_id();
			$f="<font color=blue>&lt;Successfully Updated&gt;</font>";
		}
...
<a href="#" onClick="showhide('tipsdiv')" id="mymenuitem"><img src="<?php echo $MYLIB;?>/img/help22.png"><br><?php echo $lg_readme;?></a>
	...
	<div id="mymenu_seperator"></div>
	<div id="mymenu_space">&nbsp;&nbsp;</div>
<a href="#" onClick="javascript: href='p.php?p=../sub/sub&sid=<?php echo $sid?>&clslevel=<?php echo $clslevel?>'" id="mymenuitem"><img src="<?php echo $MYLIB;?>/img/reload.png"><br><?php echo $lg_refresh;?></a>
</div>
<div align="right">
...
<?php    
		...
		$sql="select * from type where grp='classlevel' and sid='$sid' order by prm";
            ...
            while($row=mysql_fetch_assoc($res)){
                        $s=$row['prm'];
			if($s==$clslevel){$selected="selected";}else{$selected="";}
                        echo "<option value=$s $selected>$namatahap $s</option>";
            }
...
Tips:<br>
<?php if($LG=="BM"){?>
1. Silahkan Hubungi Support Untuk Mengubah Data Ini.<br>
<?php }else{?>
1. Silahkan Hubungi Support Untuk Mengubah Data Ini.<br>
<?php } ?>
</div>
<div id="panelform" <?php if($subid==""){?>style="display:none"<?php }?>>
<div id="mytitlebg">
...
<?php    
		...
		//	echo "<option value=$clslevel>$namatahap $clslevel</option>";
            ...
            while($row=mysql_fetch_assoc($res)){
                        $s=$row['prm'];
			if($clslevel==$s){$selected="selected";}else{$selected="";}
                        echo "<option value=$s $selected>$namatahap $s</option>";
            }
...
<?php	
            ...
            while($row=mysql_fetch_assoc($res)){
            	$s=$row['prm'];
				$v=$row['val'];
				if($grp==$s){$selected="selected";}else{$selected="";}
                echo "<option value=\"$s|$v\" $selected>$s</option>";
            }
?>
	...
	<tr>
			...
			<td>:</td>
            <td><input name="sub" type="text" id="sub" value="<?php echo $subname;?>"> <img src="<?php echo $MYLIB;?>/img/alert14.png" id="suberr" style="display:none"></td>
	</tr>
	<tr>
			<td><?php echo $lg_code;?></td>
			<td>:</td>
            <td><input name="cod" type="text" id="cod" size="6" value="<?php echo $subcode;?>"> <img src="<?php echo $MYLIB;?>/img/alert14.png" id="coderr" style="display:none"></td>
	</tr>
	<tr>
	...
	<tr>
			<td >KKM</td>
			<td width="1%" >:</td>
            <td><input name="sname" type="text" id="sname" size="6" maxlength="6" value="<?php echo $sname;?>"></td>
	</tr>
...
?>
	...
	<tr>
		<td>Index</td>
		<td>:</td>
		<td><input type="text" name="index" id="index" value="<?php echo $idx;?>"></td>
	</tr>
...
<?php $schname=stripcslashes($schname);?>
...
<table width="100%" cellspacing="0">
  <tr>
    ...
    <td class="mytableheader" style="border-right:none;" width="3%" align="center"><a href="#" onClick="formsort('idx','<?php echo "$nextdirection";?>')" title="Sort"><?php echo "URUTAN<br>REPORT";?></a></td>
	...
	<td class="mytableheader" style="border-right:none;" width="5%" align="center">KKM</td>
...
?>
  ...
  <tr bgcolor="<?php echo $bg;?>" style="cursor:default" onMouseOver="this.bgColor='#CCCCFF';" onMouseOut="this.bgColor='<?php echo $bg?>';">
    ...
    <td class="myborder" style="border-right:none; border-top:none;" align="center"><?php echo "$idx";?></td>
	<td class="myborder" style="border-right:none; border-top:none;" align="center">
    	<a href="#" onClick="document.myform.subid.value='<?php echo "$id";?>';
        					document.myform.submit();"><?php echo "$code";?></a></td>
...
?>
     ...
     <td class="myborder" align="center">
     	<a href="subpra.php?xcod=<?php echo $code;?>&xsid=<?php echo $xsid;?>&xlvl=<?php echo $lvl;?>" 
        	target="_blank" class="fbmedium">Set</a>
     </td>
     <?php } ?>
  </tr>
