...
<table width="100%" cellspacing="0">
  <tr >
			  ...
			  <td id="mytabletitle" width="10%" align="center">POIN PELANGGARAN</td>
			  <td id="mytabletitle" width="1%" align="center">&nbsp;</td>
			  <td id="mytabletitle" width="10%" align="center">POIN PRESTASI</td>
			  <td id="mytabletitle" width="10%" align="center">JUMLAH POIN</td>
	...
	$q=$curr;
  	while($row=mysql_fetch_assoc($res)){
		...
		if($row2=mysql_fetch_assoc($res2))
			...
			$sql="select sum(val) from dis where stu_uid='$uid' and cat='demerit'";
			$res2=mysql_query($sql)or die("query failed:".mysql_error());
			$row2=mysql_fetch_row($res2);
			$demerit=$row2[0];

			$sqlmerit="select sum(val) from dis where stu_uid='$uid' and cat='merit'";
			$resmerit=mysql_query($sqlmerit)or die("$sqlmerit query failed:".mysql_error());
			$rowmerit=mysql_fetch_row($resmerit);
			$merit=$rowmerit[0];
...
?>
	...
	<tr bgcolor="<?php echo $bg;?>" style="cursor:default" onMouseOver="this.bgColor='#CCCCFF';" onMouseOut="this.bgColor='<?php echo $bg?>';">
		...
		<td id="myborder" align=center><a href="p.php?<?php echo "p=merit_reg&uid=$uid&sid=$sid";?>"  target="_blank"><img src="../img/edit12.png"></a></td>
		<td id="myborder" align=center><a href="p.php?<?php echo "p=merit_reg&uid=$uid&sid=$sid";?>" title="Add Demerit Point"><?php echo $merit;?></a></td>
