	...
	if($op=='delete'){	
		if (count($del)>0) {
			for ($i=0; $i<count($del); $i++) {
		      	$sql="delete from hos where id=$del[$i]";
		      	mysql_query($sql)or die("query failed:".mysql_error());
			}
		}
		echo "<script language=\"javascript\">location.href='prmhos.php?grp=$grp'</script>;";
	}
	else{
		if($id!="")
			$sql="update type set val=$val,idx=$idx,des='$des',code='$code',lvl=$lvl,sid=$sid,etc='$etc' where id=$id";
		else{
      		$sql="insert into hos (name,block,level,noroom,nostu,sid,code,sex) values ('$name','$block','$level','$noroom','$nostu','$sid','$code','$sex')";
			mysql_query($sql)or die("query failed:".mysql_error());
			for($i=1;$i<=$noroom;$i++){
				for($j=1;$j<=$nostu;$j++){
					$sql="insert into hos_room (name,block,level,roomno,stuno,sid,code,sex) values ('$name','$block','$level','$i','$j','$sid','$code','$sex')";
					mysql_query($sql)or die("query failed:".mysql_error());
				}
			}
		}
...
<?php
	$sql02="select * from hos order by name, level";
	...
	while($row=mysql_fetch_assoc($res02)){
		...
		echo "<td><a href=\"prmhos.php?id=$xid\" title=\"edit\" style=\"text-decoration:underline\">$block</a></td>";
