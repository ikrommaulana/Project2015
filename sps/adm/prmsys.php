...
if($op==""){
	if($id!=""){
		$sql="select * from sys_prm where id=$id";
		$res=mysql_query($sql)or die("query failed:".mysql_error());
		$row=mysql_fetch_assoc($res);
		$prm=$row['prm'];
		$val=$row['val'];
	...
	$sql="select * from sys_prm where grp='$grp'";
	$res=mysql_query($sql)or die("query failed:".mysql_error());
	$row=mysql_fetch_assoc($res);
	$status=$row['sta'];
}
	...
	if($op=='delete'){	
		if (count($del)>0) {
			for ($i=0; $i<count($del); $i++) {
		      	$sql="delete from sys_prm where id=$del[$i]";
		...
		if($id!="")
			$sql="update sys_prm set prm='$prm',val=$val,idx=$idx,des='$des',etc='$etc',code='$code',lvl=$lvl,sid=$sid,adm='$username',ts=now() where id=$id";
		else
      		$sql="insert into sys_prm (prm,val,grp,idx,des,etc,code,lvl,sid,adm,ts) values ('$prm','$val','$grp','$idx','$des','$etc','$code',$lvl,$sid,'$username',now())";
		mysql_query($sql)or die("query failed:".mysql_error());
		$f="<font color=blue>&lt;SUCCESSFULY UPDATE&gt;</font>";
		$id="";
	}
...
<div id="mytitle">
	<div id="myclick" onClick="hide('panelform');"><img src="../img/icon_minimize.gif" id="mycontrolicon">SYSTEM CONFIGURATION</div>&nbsp;
</div>
	...
	<div id="mytitle">SYSTEM SETTING : <?php echo $grp;?> <?php echo $f;?></div>
...
<?php
	$sql="select count(*) from sys_prm where grp='$grp' $sqlsid";
	...
	$sql="select * from sys_prm where grp='$grp'  $sqlsid $sqlsort $sqlmaxline";
