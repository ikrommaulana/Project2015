...
$username = $_SESSION['username'];
	...
	if($operation=="delete"){
		...
		echo "<script language=\"javascript\">location.href='p.php?p=stureg&st=2'</script>";
			...
			$sql="update stu set name='$name',ic='$ic',acc='$acc',sex='$sex',nogiliran='$nogiliran',etc='$etc',
					...
					isstaff=$isstaff,isyatim=$isyatim,isfeenew=$isfeenew,isfeeonanak=$isfeeonanak,isfakir=$isfakir,isspecial=$isspecial,
		...
		if($num>0){
			...
			$status=6;//pelajar
			$sql="insert into stu(cdate,sch_id,clslevel,pass,name,ic,sex,acc,
				...
				q0,q1,q2,q3,q4,q5,q6,q7,q8,q9,p1name,p1ic,p1rel,p1job,p1sal,p1com,
				 ...
				 pschool,transport,isstaff,isyatim,iskawasan,isfeenew,isfeeonanak,
				 isfeefree,feehutang,isfakir,isspecial,isinter,mentor,etc,isblock,adm,ts,sponser,bstate) values
				(now(),$sid,$clslevel,'$pass','$name','$ic','$sex','$acc',
				...
				$isstaff,$isyatim,$iskawasan,$isfeenew,$isfeeonanak,$isfeefree,'$feehutang',
				$isfakir,$isspecial,$isinter,'$mentor','$etc',$isblock,'$username',now(),'$sponser','$bstate')";
				//echo $sql;
