...
<div id="mypanel">
        ...
        <select name="month" onChange="document.myform.sdt.value='';document.myform.edt.value='';document.myform.submit();">
                <?php
...
<?php          
	$sql="select * from stureg where sch_id=$sid and studentstatus>10 $sqlsearch $sqlsort";
	$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
	$q=$curr;
  	while($row=mysql_fetch_assoc($res)){   
			$name=ucwords(strtolower(stripslashes($row['name'])));
			$sex=$lg_sexmf[$row['sex']];
			$nobukuinduk=$row['nobukuinduk']; 
			$xsid=$row['sch_id']; 
			$ic=$row['ic'];
			$enddate=$row['enddate'];
...
<?php          
	$sql="select * from stureg where sch_id=$sid and status=100 $sqlsearch $sqlsort";
	$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
	$q=$curr;
  	while($row=mysql_fetch_assoc($res)){   
			$name=ucwords(strtolower(stripslashes($row['name'])));
