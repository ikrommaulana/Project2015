...
if($op==""){
	if($id!=""){
		$sql="select * from koq where id=$id";
		$res=mysql_query($sql)or die("query failed:".mysql_error());
		$row=mysql_fetch_assoc($res);
		$prm=$row['prm'];
		$val=$row['val'];
	...
	$sql="select * from koq where grp='$grp'";
	$res=mysql_query($sql)or die("query failed:".mysql_error());
	$row=mysql_fetch_assoc($res);
	$status=$row['sta'];
}
	...
	if($op=='delete'){	
		if (count($del)>0) {
			for ($i=0; $i<count($del); $i++) {
		      	$sql="delete from koq where id=$del[$i]";
		...
		if($id!="")
			$sql="update koq set prm='$prm',val=$val,idx=$idx,des='$des',etc='$etc',code='$code',lvl=$lvl,sid=$sid,adm='$username',ts=now() where 
id=$id";
		else
      		$sql="insert into koq (prm,val,grp,idx,des,etc,code,lvl,sid,adm,ts) values ('$prm','$val','$grp','$idx','$des','$etc','$code',$lvl,$sid,'$username',now())";
		mysql_query($sql)or die("query failed:".mysql_error());
		$f="<font color=blue>&lt;SUCCESSFULY UPDATE&gt;</font>";
		$id="";
	}
...
<link rel="stylesheet" href="../inc/my2.css" type="text/css">
</head>
<body >
...
<?php
	$sql="select count(*) from koq where grp='$grp' $sqlsid";
	...
	$sql="select * from koq where grp='$grp'  $sqlsid $sqlsort $sqlmaxline";
