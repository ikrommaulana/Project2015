...
<div id="story">
<div id="mytitlebg"><?php echo strtoupper("$lg_cocurriculum");?> - <?php echo strtoupper($sname);?> <?php echo $year;?></div>
...
<?php 
    ...
    while($row2=mysql_fetch_assoc($res2)){
		 $s=$row2['prm'];
		 $sql="select count(*) from stu INNER JOIN (ses_stu,koq_stu) ON stu.uid=ses_stu.stu_uid and stu.uid=koq_stu.uid where stu.sch_id=$sid and stu.status=6 and stu.sex='Lelaki' and koq_stu.koq_name='$koq' and koq_stu.sta=0 and ses_stu.cls_level=$s  and koq_stu.year='$year'";
		 $res3=mysql_query($sql)or die("$sql failed:".mysql_error());
    	 $row3=mysql_fetch_row($res3);
		 $numl=$row3[0];
		 $sql="select count(*) from stu INNER JOIN (ses_stu,koq_stu) ON stu.uid=ses_stu.stu_uid and stu.uid=koq_stu.uid where stu.sch_id=$sid and stu.status=6  and stu.sex='Perempuan' and koq_stu.koq_name='$koq' and koq_stu.sta=0 and ses_stu.cls_level=$s  and koq_stu.year='$year'";
		 $res3=mysql_query($sql)or die("$sql failed:".mysql_error());
    	 $row3=mysql_fetch_row($res3);
		 $nump=$row3[0];
