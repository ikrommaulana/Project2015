...
$adm = $_SESSION['username'];
	...
	if($op=='save'){
		...
		$sql="update koq_stu set dts='$dts',dte='$dte',koq_name='$kname',koq_type='$ktype',year='$year',koq_code='$kcode',
	...
	if($id!=""){
		...
		$sqlstukoq="select * from type where grp='koq_jawatan' and code='setkoq' and prm='$pos'";
		$resstukoq=mysql_query($sqlstukoq)or die("$resstukoq query failed:".mysql_error());
		$rowstukoq=mysql_fetch_assoc($resstukoq);
	}
...
function process_form(action){
	...
	if(action=='save'){
		...
		if(document.myform.pos.value==""){
			alert("Choose Position");
			document.myform.pos.focus();
			return;
		}
...
<div id="mypanel">
	<div id="mymenu" align="center">
		...
		<a href="#" onClick="javascript: href='koq_stu_reg.php?sid=<?php echo $sid;?>&uid=<?php echo $uid;?>&year=<?php echo $year;?>'" id="mymenuitem"><img src="../img/reload.png"><br>Refresh</a>
...
<?php } ?>
	...
	<table width="100%" >
	  ...
	  <tr>
        ...
        <td>:</td>
        <td><?php echo strtoupper("$cname / ($year1)");?> </td>
      </tr>
    </table>
...
<table width="100%" cellspacing="0" cellpadding="5">
  ...
  <tr>
	<td width="10%" id="myborder">Nilai</td>
	<td width="90%" id="myborder">
		<select name="pos">
			<?php
			if($pos==""){
				echo "<option value=\"\">- Pilih Nilai -</option>";
			}
			$sqlsetkoq="select * from type where grp='koq_jawatan' and code='setkoq'";
			$ressetkoq=mysql_query($sqlsetkoq)or("$sqlsetkoq query failed:");
			while($rowsetkoq=mysql_fetch_assoc($ressetkoq)){
				$prmkoq=$rowsetkoq['prm'];
				if($prmkoq==$pos){$selected="selected";}else{$selected="";}
				echo "<option value=\"$prmkoq\" $selected>$prmkoq</option>";
			}
			?>
		</select>
	</td>
...
<table width="100%" cellspacing="0" cellpadding="0">
  <tr>
		...
		<td id="mytabletitle" width="15%" align="center"><?php echo strtoupper($lg_remark);?></td>
  </tr>
<?php
	...
	while($row=mysql_fetch_assoc($res)){
		...
		$sqldes="select * from type where grp='koq_jawatan' and code='setkoq' and prm='$pos'";
...
?>
	...
	<td id="myborder" align="center"><?php echo "$pos";?></td>
	<td id="myborder" align="center"><?php echo "$par_des";?></td>
