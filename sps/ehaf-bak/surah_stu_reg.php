...
verify('ADMIN|AKADEMIK|KEWANGAN|GURU|HR|SOKONGAN');
		...
		if($op=="save"){
				...
				$sql="insert into surah_stu_status(ts,adm,dt,uid,sid,surahno,surahayat,surahname,reading,status) 
					values (now(),'$adm','$dt','$uid',$sid,$surahno,$ayatno,'$surahname',$reading,$status)";
				$res=mysql_query($sql)or die("$sql - query failed:".mysql_error());
				$f="Successfully Save";
		}
		elseif($op=="delete"){
		...
		$sql="select * from surah_stu_read where uid='$uid' and surahno=$surahno and reading='1' order by id desc";
		$res2=mysql_query($sql)or die("query failed: $sql".mysql_error());
		$row2=mysql_fetch_assoc($res2);
...
<td width="33%">
<input type="radio" name="reading" value="0" <?php if($reading=="0") echo checked;?> onClick="document.myform.submit();"> HAFAZAN 
