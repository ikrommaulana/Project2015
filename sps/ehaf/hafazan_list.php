...
$op=$_REQUEST['op'];
if($op=="delete"){
		$sql="update hafaz_construct set isdel=1,delby='$adm',delts=now() where id='$edit'";
...
if($op=="save"){
		...
		if($code==""){
				$sql="select count(*) from hafaz_construct";
				$res=mysql_query($sql)or die("$sql query failed:".mysql_error());
		...
		if($edit>0){
			$sql="update hafaz_construct set sid='$sid',cate='$cate',cate='$grading',type='$type',code='$code',item='$item',exam='$exam',lvl='$lvl',adm='$adm',ts=now() where id=$edit";
			$res=mysql_query($sql)or die("$sql query failed:".mysql_error());		
		}else{
			$sql="insert into hafaz_construct (sid,cate,type,code,item,grading,exam,lvl,adm,ts)values
				('$sid','$cate','$type','$code','$item','$grading','$exam','$lvl','$adm',now())";
			$res=mysql_query($sql)or die("$sql query failed:".mysql_error());
...
if($edit>0){
			$sql="select * from hafaz_construct where id='$edit'";
			$res=mysql_query($sql)or die("query failed:".mysql_error());
  			$row=mysql_fetch_assoc($res);
			$xid=$row['id'];
			$sid=$row['sid'];
                	 ...
                	 <tr onMouseOver="this.bgColor='#F1F1F1'" onMouseOut="this.bgColor=''">
                        ...
                        <select name="cate">
								<?php 
                                    ...
                                    $sql="select * from type where grp='hafaz_cate' and code!='1' order by idx,prm";
                                    $res=mysql_query($sql)or die("query failed:".mysql_error());
                        ...
                        <input type="button" value="+" onClick="newwindow('../adm/prm.php?grp=hafaz_cate',0)">
                        </td>
				  	</tr>
                	 <tr onMouseOver="this.bgColor='#F1F1F1'" onMouseOut="this.bgColor=''">
                        ...
                        <select name="type">
								<?php 
                                   // if($type!="")
                                    ...
                                    //else
                                                echo "<option value=\"\">- $lg_select -</option>";
                                    $sql="select * from type where grp='hafaz_type' order by prm";
                                    $res=mysql_query($sql)or die("query failed:".mysql_error());
                                    while($row=mysql_fetch_assoc($res)){
                                                $s=$row['prm'];
												$t=stripslashes($s);
												if($t==$type){$selected="selected";}else{$selected="";}
                                                echo "<option value=\"$t\" $selected>$t</option>";
                                    }
                                ?>
                        </select>
                        <input type="button" value="+" onClick="newwindow('../adm/prm.php?grp=hafaz_type',0)">
                        </td>
				  </tr>                 
				  <tr onMouseOver="this.bgColor='#F1F1F1'" onMouseOut="this.bgColor=''">
...
<?php
		...
		$res2=mysql_query($sql)or die("query failed:".mysql_error());
      	while($row2=mysql_fetch_assoc($res2)){
				$type1=$row2['type'];
				$types=stripslashes($type1);
				$jj++;
				$bg="#FAFAFA";
?>				
            <tr bgcolor="<?php echo $bg;?>" style="cursor:default;" onMouseOver="this.bgColor='#CCCCFF';" onMouseOut="this.bgColor='<?php echo $bg?>';">                          
                            <td id="myborder" style="border-top:none;border-right:none;" align="right"><?php echo "$q-$jj";?></td>
                            <td id="myborder" style="border-top:none;border-right:none;"><?php echo $types;?></td>
						...
						?>
                                <tr bgcolor="<?php echo $bg;?>" style="cursor:default;" onMouseOver="this.bgColor='#CCCCFF';" onMouseOut="this.bgColor='<?php echo $bg?>';">                          
                                    ...
                                    <td id="myborder" style="border-top:none;border-right:none;" align="center">
                                    		<a href="../adm/p.php?p=../ehaf/hafazan_list&edit=<?php echo $xid;?>"><img src="<?php echo $MYLIB;?>/img/edit12.png"></a>&nbsp;
                    						<a href="#" onClick="process_form('delete',<?php echo $xid;?>)" title="Delete"><img src="<?php echo $MYLIB;?>/img/delete12.png"></a>
                                    </td>
                           </tr>
<?php } ?>
<?php } ?> 
