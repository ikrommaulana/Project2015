...
$p=$_REQUEST['p'];
		...
		**/
		if($clslevel!="")
                ...
                $exam=$_POST['exam'];
		if($exam!==""){
			$open=mysql_query("SELECT * FROM type where grp='examnas' and code='$exam'") or die (mysql_error());
			$read=mysql_fetch_array($open);
			$examname=$read['code'];
		}
				...
				?>
					...
					?>
                                    ...
                                    <select name="exam" onchange="document.myform.submit();">
                                        <?php	
                                        if($exam=="")	
                                                echo "<option value=\"\">- Ujian -</option>";
                                        else
                                                echo "<option value=\"$exam\">$examname</option>";
                                                        $sql="select * from type where grp='examnas' and code!='$exam' order by idx";
                                        $res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
                                                while($row=mysql_fetch_assoc($res)){
                                        $a=$row['prm'];
                                                                $b=$row['code'];
...
<table width="100%" cellspacing="0" cellpadding="0">
 <tr> 
    ...
    <td id="mytabletitle" width="20%" align="left">Input Qiroati</td>
    <td id="mytabletitle" width="20%" align="left">Cetak Raport</td>
 </tr>
	<?php	
...
?>
		...
		<td id="myborder"><?php echo "$name";?></td>
                <td id="myborder" align="center">
                    <?php
                            $sql="select count(*) from surah_stu_status where uid='$uid' and exam='$exam' and sid='$sid' and year='$year' and sem='$smt' and lvl='$clslevel' and grp='qiroati'";
                            ...
                            if($mm>0)
                                    ...
                                    $img="../img/edit12.png";
                    ?>
                    <a href="input_qiroati.php?<?php echo "lvl=$clslevel&year=$year&exam=$exam&sid=$sid&smt=$smt&uid=$uid";?>" id="pos_bottom<?php echo $q;?>" target="_blank"
                    <div id="checkimg<?php echo $q;?>"><img src="<?php echo $img;?>"></div></a>
                </td>
                <td id="myborder" align="center"><a href="/raport_qiroati.php?<?php echo "clscode=$clscode&year=$year&exam=$exam&sid=$sid&smt=$smt&uid=$uid";?>" target="_blank" title="Cetak Raport"><img src="../img/printer12.png"></a></td>
		</tr>
<?php  }  ?>
     </table>       
