...
verify('ADMIN|AKADEMIK|KEUANGAN|GURU|HR');
...
$year=$sesyear;
		...
		$isasuh=$_REQUEST['isasuh'];
		if($isasuh!=""){
			$sqlisasuh="and isasuh=1";
			$x1="- Anak Asuh";
		}
...
<div id="mytabletitle" class="printhidden" style="padding:5px 5px 5px 5px;margin:0px 1px 0px 1px;" align="right">
	...
	<?php if((is_verify("ROOT|ADMIN|AKADEMIK|KEUANGAN|HR|CEO"))||($SHOW_EPELAJAR_TO_ALL_TEACHER)){?>
...
<?php } ?>
			 <select name="clscode" id="clscode" onchange="document.myform.submit();">
                  <?php	
					...
					$sql="select * from ses_cls where sch_id=$sid and cls_code!='$clscode' and year='$sesyear' $sqlclslevel order by cls_level";
...
?>
	...
	<label style="cursor:pointer"><input type="checkbox" name="isasuh" value="1" <?php if($isasuh) echo "checked";?>><?php echo "Anak Asuh";?></label>&nbsp;
...
<table width="100%" border="0" >
  ...
  <tr>
    ...
    <td><?php echo "Kartu Tanda Penduduk";?></td>
  </tr>
  <tr>
...
<?php	
	if(($clscode=="")&&($clslevel==""))
    	$sql="select count(*) from stu where sch_id=$sid $sqlstustatus $sqlisyatim $sqlisasuh $sqlisinter $sqlisstaff $sqliskawasan $sqlisislah $sqlishostel $sqlisfakir $sqlsearch";
	else
		$sql="select count(*) from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where ses_stu.sch_id=$sid $sqlclscode $sqlclslevel $sqlstustatus $sqlisyatim $sqlisasuh $sqlisinter $sqlisstaff $sqliskawasan $sqlisislah $sqlishostel $sqlisfakir $sqlsearch and year='$year'";
   echo "$sql";
	...
	if(($clscode=="")&&($clslevel==""))
		$sql="select * from stu where sch_id=$sid $sqlstustatus $sqlisyatim $sqlisasuh $sqlisblock $sqlisstaff $sqlisinter $sqliskawasan $sqlisislah $sqlisfakir $sqlishostel $sqlsearch $sqlsort $sqlmaxline";
	else
		$sql="select stu.*,ses_stu.cls_name from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where ses_stu.sch_id=$sid $sqlstustatus $sqlclscode $sqlclslevel $sqlisyatim $sqlisasuh $sqlisinter $sqlisblock $sqlisstaff $sqliskawasan $sqlisislah $sqlisfakir $sqlishostel $sqlsearch and year='$year' $sqlsort $sqlmaxline";
	...
	$q=$curr;
  	while($row=mysql_fetch_assoc($res)){
		...
		echo "<tr bgcolor=$bg style=\"cursor:pointer\" onMouseOver=\"this.bgColor='#CCCCFF';\" onMouseOut=\"this.bgColor='$bg'\" onClick=\"newwindow('../stu/stu_info.php?p=stu_profile&uid=$uid&sid=$sid',0)\">";
...
<table width="100%" cellspacing="0" cellpadding="2">
	<tr >
	      ...
	      <?php if(is_verify("ROOT|ADMIN")){ ?>
			  <td class="mytableheader" style="border-right:none;" width="5%" align="center" colspan="2"><a href="#" onClick="formsort('uid','<?php echo "$nextdirection";?>')" title="Sort"><?php echo strtoupper($lg_matric);?></a></td>
		<?php } else { ?>
		...
		<?php } ?>
			 <td class="mytableheader" style="border-right:none;" width="5%" align="center"><a href="../stu/addnisn.php?sid=<?php echo $sid ?>&year=<?php echo $year ?>&clslevel=<?php echo $clslevel ?>&clscode=<?php echo $clscode ?>" style="text-decoration:none" target="_blank">NISN</a></td>
			  ...
			  <td class="mytableheader" style="border-right:none;" width="10%"><?php echo strtoupper("$lg_birth_no / Passport $lg_student");?></td>
			  ...
			  <td class="mytableheader" style="border-right:none;" width="10%" align="center"> <?php echo strtoupper("KTP orang tua");?></td>
			  <td class="mytableheader"  width="7%" align="center">STATUS</td> 
              <td class="mytableheader"  width="7%" align="center">BUKU INDUK</td> 
	</tr>
<?php	
	if(($clscode=="")&&($clslevel==""))
    	$sql="select count(*) from stu where sch_id=$sid $sqlstustatus $sqlisyatim $sqlisasuh $sqlisinter $sqlisstaff $sqliskawasan $sqlisislah $sqlishostel $sqlisfakir $sqlsponser $sqlsearch";
	else
		$sql="select count(*) from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where ses_stu.sch_id=$sid $sqlclscode $sqlclslevel $sqlstustatus $sqlisyatim $sqlisasuh $sqlisinter $sqlisstaff $sqliskawasan $sqlisislah $sqlishostel $sqlisfakir $sqlsponser $sqlsearch and year='$year'";
    $res=mysql_query($sql,$link)or die("$sql:query failed:".mysql_error());
    $row=mysql_fetch_row($res);
    $total=$row[0];
	...
	if(($clscode=="")&&($clslevel==""))
	$sql="select * from stu where sch_id=$sid  $sqlstustatus $sqlisyatim $sqlisasuh $sqlisblock $sqlisstaff $sqliskawasan $sqlisislah $sqlisinter $sqlisfakir $sqlishostel $sqlsponser $sqlsearch $sqlsort $sqlmaxline";
	//$sql="select stu.*,ses_stu.cls_name,ses_stu.year from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where ses_stu.sch_id=$sid and ses_stu.year='$year' $sqlsearch $sqlsort $sqlmaxline";

	else
		$sql="select stu.*,ses_stu.cls_name,ses_stu.year from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where ses_stu.sch_id=$sid $sqlstustatus $sqlclscode $sqlclslevel $sqlisyatim $sqlisasuh $sqlisinter $sqlisblock $sqlisstaff $sqliskawasan $sqlisislah $sqlisfakir $sqlishostel $sqlsponser $sqlsearch and year='$year' $sqlsort $sqlmaxline";
//echo $sql;
	$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
	$q=$curr;
	...
	//echo $sql;
  	while($row=mysql_fetch_assoc($res)){
		...
		$sql="select stu.* from stu left join stureg on stu.uid=stureg.uid where stu.id='$id' and stu.sch_id=$sid";
...
<?php
    	 if(is_verify("ROOT|ADMIN")){
				echo "<a href=\"../stu/stu_info.php?p=stu_profile&uid=$uid&sid=$sid&year=$year\" title=Profile target=_blank onClick=\"return GB_showPage('Profile : $xxname ',this.href)\"><img src=\" $MYLIB/img/user12.png\"></a>
				<a href=\"../stu/stureg.php?uid=$uid&sid=$sid\" title=Edit target=_blank title='Edit Profile : $xxname '\"><img src=\" $MYLIB/img/edit12.png\"></a>
				...
				";
		}elseif(is_verify("KEUANGAN")){
				echo "<a href=\"../stu/stu_info.php?p=stu_profile&uid=$uid&sid=$sid\" title=Profile target=_blank onClick=\"return GB_showPage('Profile : $xxname ',this.href)\"><img src=\" $MYLIB/img/user12.png\"></a>
				<a href=\"../efee/$FN_FEEPAY.php?uid=$uid&year=$year&sid=$sid\" title=Payment target=_blank onClick=\"return GB_showPage('Fee : $xxname',this.href)\"><img src= $MYLIB/img/dollar12.png></a> 
				";
		}
		...
		echo "<td class=myborder style=\"border-right:none; border-top:none;\" align=center>$inyear</td>";
		if(is_verify("ROOT|ADMIN")){ 
		echo "<td class=myborder style=\"border-right:none; border-top:none;\" align=center><a href=\"../stu/stu_uid.php?id=$id&sid=$sid\" title=Edit target=_blank title='Edit Profile : $xxname '\"><img src=\" $MYLIB/img/edit12.png\"></a></td>";
		}
		echo "<td class=myborder style=\"border-right:none; border-top:none;\" align=center>$uid</td>";
		echo "<td class=myborder style=\"border-right:none; border-top:none;\" align=center>$nisn</td>";
		echo "<td class=myborder style=\"border-right:none; border-top:none;\" align=\"center\">$sex</td>";
 		echo "<td class=myborder style=\"border-right:none; border-top:none;\"><a href=\"../stu/stu_info.php?p=stu_profile&uid=$uid&sid=$sid\" title=Profile target=_blank onClick=\"return GB_showPage('Profile : $xxname ',this.href)\">$name</a></td>";
		...
		echo "<td class=myborder style=\"border-right:none; border-top:none;\"><a href=\"../edaftar/bukuinduk.php?sid=$sid&ic=$ic&uid=$uid\" title=Profile target=_blank>$lg_view</a></td>";
		echo "</tr>";
  }
