...
<table width="100%" cellspacing="0">
  <tr >
	...
	/*if($block!=""){
		...
		if(($clscode=="")&&($tahap==""))
			echo"1".$sql="select count(*) from stu INNER JOIN hos_room ON stu.uid=hos_room.uid where stu.sch_id=$sid and ishostel=1 $sqlblock $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch order by hos_room.level,hos_room.roomno,hos_room.stuno";
		else
			echo"2".$sql="select count(*) from stu INNER JOIN (ses_stu,hos_room) ON stu.uid=ses_stu.stu_uid and stu.uid=hos_room.uid where stu.sch_id=$sid and ishostel=1 $sqlblock $sqlclslevel $sqlclscode $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch and year='$year' ";
		...
		echo"3".$sql="select count(*) from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where stu.sch_id=$sid and ishostel=1 $sqlclslevel $sqlclscode $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch and year='$year'";	
	...
	if($block!=""){
		...
		if(($clscode=="")&&($tahap==""))
			echo"4".$sql="select stu.* from stu INNER JOIN hos_room ON stu.uid=hos_room.uid where stu.sch_id=$sid and ishostel=1 $sqlblock $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch order by hos_room.level,hos_room.roomno,hos_room.stuno limit $curr,$MAXLINE";
		else
			echo"5".$sql="select stu.*,ses_stu.cls_name from stu INNER JOIN (ses_stu,hos_room) ON stu.uid=ses_stu.stu_uid and stu.uid=hos_room.uid where stu.sch_id=$sid and ishostel=1 $sqlblock $sqlclslevel $sqlclscode $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch and year='$year' $sqlsort limit $curr,$MAXLINE";
	}else if(($clscode=="")&&($tahap==""))
	
		echo"6".$sql="select * from stu where sch_id=$sid and ishostel=1 $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch $sqlsort limit $curr,$MAXLINE";
	else
		echo"7".$sql="select stu.*,ses_stu.cls_name from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where stu.sch_id=$sid and ishostel=1 $sqlclslevel $sqlclscode $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch and year='$year' $sqlsort limit $curr,$MAXLINE";
	...
	if($block!=""){
		...
		if(($clscode=="")&&($tahap==""))
			...
			$sql="select count(*) from stu INNER JOIN hos_room ON stu.ic=hos_room.ic where stu.sch_id=$sid $sqlblock $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch order by hos_room.level,hos_room.roomno,hos_room.stuno";
		else

			$sql="select count(*) from stu INNER JOIN (ses_stu,hos_room) ON stu.ic=ses_stu.stu_ic and stu.ic=hos_room.ic where stu.sch_id=$sid  $sqlblock $sqlclslevel $sqlclscode $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch and year='$year' ";
		...
		$sql="select count(*) from stu INNER JOIN ses_stu ON stu.ic=ses_stu.stu_ic where stu.sch_id=$sid  $sqlclslevel $sqlclscode $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch and year='$year'";	
	...
	if($block!=""){
		...
		if(($clscode=="")&&($tahap==""))
			...
			$sql="select stu.* from stu INNER JOIN hos_room ON stu.ic=hos_room.ic where stu.sch_id=$sid $sqlblock $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch order by hos_room.level,hos_room.roomno,hos_room.stuno limit $curr,$MAXLINE";

		else

			$sql="select stu.*,ses_stu.cls_name from stu INNER JOIN (ses_stu,hos_room) ON stu.uid=ses_stu.stu_uid and stu.uid=hos_room.uid where stu.sch_id=$sid $sqlblock $sqlclslevel $sqlclscode $sqlstustatus $sqlisyatim $sqlisstaff $sqliskawasan $sqlisfakir $sqlsearch and year='$year' $sqlsort limit $curr,$MAXLINE";
...
?>
	<tr bgcolor="<?php echo $bg;?>" style="cursor:default" onMouseOver="this.bgColor='#CCCCFF';" onMouseOut="this.bgColor='<?php echo $bg?>';">
		...
		<td id="myborder" align="center">
			<a href="letter.php?uid=<?php echo "$uid&lid=hostel_registration";?>" title="Hostel Registration" rel="gb_page_center[1000, 480]" target="_blank">AKUAN</a>
			/
			<a href="letter.php?uid=<?php echo "$uid&lid=hostel_slip";?>" title="Hostel_slip" rel="gb_page_center[1000, 480]" target="_blank">SLIP DAFTAR</a>
		</td>
		</tr>
<?php }  ?>
     </table>          
