...
<?php 
$chart_note_top="";
$chart_note_left="Bilangan Pelajar";
$chart_item_group="";//"Rendah,Menengah,Tahfiz,Tadika";
$sql="select * from type where grp='pusattemuduga' order by prm";
...
$chart_item_value="Pusat Temuduga";
$sql="select * from type where grp='pusattemuduga' order by prm";
$res=mysql_query($sql)or die("query failed:".mysql_error());
while($row=mysql_fetch_assoc($res)){
		$s=$row['prm'];
		$sql="select count(*) from stureg where sesyear='$sesyear' and pt='$s' and confirm=1";
...
$chart_note_left="Pusat Temuduga";
...
while($row=mysql_fetch_assoc($res)){
		...
		$sql="select count(*) from stureg where pt='$s' and sesyear=$sesyear and sex=1 and confirm=1";
		$res2=mysql_query($sql)or die("query failed:".mysql_error());
		$row2=mysql_fetch_row($res2);
		$t=$row2[0];
		if($totalmale!="")
		...
		$sql="select count(*) from stureg where pt='$s' and sesyear=$sesyear and sex=0 and confirm=1";
		$res2=mysql_query($sql)or die("query failed:".mysql_error());
		$row2=mysql_fetch_row($res2);
		$t=$row2[0];
...
<table width="100%" cellpadding="2" cellspacing="0">
	<tr>
        <td id="mytabletitle">SESI / KAUNTER</td>
<?php
$sql="select * from type where grp='pusattemuduga' order by prm";
$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
while($row=mysql_fetch_assoc($res)){
		$pt=$row['prm'];
?>
        <td  id="mytabletitle"><?php echo $pt;?></td>
<?php }?>
    </tr>
 <?php
$sql="select * from type where grp='clssession' order by prm";
$res2=mysql_query($sql)or die("$sql:query failed:".mysql_error());
while($row2=mysql_fetch_assoc($res2)){
		$ss=$row2['prm'];
?> 
	<tr>
        <td id="mytabletitle"><?php echo $ss;?></td>
<?php
$sql="select * from type where grp='pusattemuduga' order by prm";
$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
while($row=mysql_fetch_assoc($res)){
		$pt=$row['prm'];
		$sql="select count(*) from stureg where pt='$pt' and clssession='$ss' and sesyear=$sesyear and confirm=1";
		$res3=mysql_query($sql)or die("query failed:".mysql_error());
		$row3=mysql_fetch_row($res3);
		$t=$row3[0];
?>
        <td id="myborder" style="font-weight:bold;"><?php echo $t;?></td>
<?php }?>
    </tr>
<?php }?>
</table>
