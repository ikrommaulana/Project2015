...
if($op=="save"){
		...
		if($edit>0){
			$sql="update stu_summary set year='$year',lvl='$lvl',sem='$sem',cls='$cls',rem1='$rem1',adm='$adm',ts=now() where id=$edit";
		}else{
			$sql="insert into stu_summary (type,sid,ic,year,lvl,sem,cls,rem1,adm,ts)
				values('SCHOLAR','$sid','$ic','$year','$lvl','$sem','$cls','$rem1','$adm',now())";
		}
...
<div id="mytitle2">Kehadiran <?php echo $f;?></div>
				...
				<table width="100%" id="mytitle" cellspacing="0" cellpadding="2px" style="font-size:11px">
                    ...
                    <tr onMouseOver="this.bgColor='#F1F1F1'" onMouseOut="this.bgColor=''">
                        <td id="myborderbottom">Dari</td>
                        <td width="80%"><input name="rem1" type="text" value="<?php echo $rem1;?>"></td>
				  	</tr>
