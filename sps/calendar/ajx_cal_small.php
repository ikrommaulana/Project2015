...
$year= date("Y");     // Finds today's year
			...
			//echo $sqlmonth="select * from month where no='$monthminus' $month";
...
function process_calendar(prm,chm,tar){
		...
		xmlhttp.onreadystatechange=function(){
			if (xmlhttp.readyState==4 && xmlhttp.status==200){
				document.getElementById("cal_small_div").innerHTML=xmlhttp.responseText;
		...
		xmlhttp.open("GET","../calendar/ajx_cal_small.php?prm="+prm+"&chm="+chm+"&tar="+tar,true);
		xmlhttp.send();
}
...
<table align=center width='100%' cellspacing="0">
   <tr>
	  	...
	  	<td colspan=3 align=center id="masthead_title" style="padding:3px;border:none;">
			<?php
			
			$monthno=date('n',mktime(0,0,0,$month,1,$year));
			$sqlmonthname="select * from month where no='$monthno'";
			$resmonthname=mysql_query($sqlmonthname)or die("$sqlmonthname query failed:".mysql_error());
			$rowmonthname=mysql_fetch_assoc($resmonthname);
			$monthname=$rowmonthname['name'];
...
<table align="center" width='100%' cellspacing="0" cellpadding="3px">
	<tr>
	  ...
	  <td width="14%" id="myborder" style="border-color:#999999;border-right:none;font-size:11px;" align="center"  background="<?php echo $MYLIB;?>/img/bg_title_lite.jpg"><b>Sen</b></td>
...
?>
		<td id="myborder"  <?php echo "$bg";?>align="center" style="border-color:#999999;border-right:none;font-size:11px;border-top:none;<?php echo "$br";?>" 
			...
			<a href="../calendar/cal_big.php?xdate=<?php echo "$dt&prm=$prm&chm=$chm";?>" class="fbbig" target="_blank"><?php echo "$date";?></a>
		</td>
<?php	
		$number_of_cell++;
		if($j==7){
...
?>
	...
	<div style="font-size:11px;  font-weight:bold; border:1px solid #ccc; padding:4px;background-image:url(<?php echo $MYLIB;?>/img/bg_panel2.jpg)">
			<?php echo $m_ind[$month]; ///date('F',mktime(0,0,0,$month,1,$year));?><?php if($LG=="BI") echo "'s ";?>
			<?php echo "Program & Aktifitas";?>
    </div>
            
            
            
	<div id="myborder" style="height:150px; min-height:150px; overflow-y: auto;background-image:url(<?php echo $MYLIB;?>/img/bg_panel2.jpg);">
<?php
		$i=0;
...
?>
	<div  style="font-size:9px; padding:5px 4px 5px 4px">
            ...
            <br><?php
	    $day_e=date('l',mktime(0,0,0,$xmm,$xdd,$xyy));
	    
	  //  echo $days[$xdt];
	    echo " $xdd/$xmm/$xyy";?></font>
	</div>
...
?>
	<div  style="font-size:9px; padding:5px 4px 5px 4px">
            ...
            <?php if($isprivate){?><img src="<?php echo $MYLIB;?>/img/user12.png" height="10" title="Personal Reminder"><?php } ?>
            <br><?php
	    $dateL=date('w',mktime(0,0,0,$xmm,$xdd,$xyy));
	    echo $days[$dateL];
