...
$yearcontent=$_REQUEST['yearcontent'];
if($yearcontent!=""){
	$yearcont="and year='$yearcontent'";
} 
?>
...
<h2>Pilih tahun ajaran terlebih dahulu</h2> <select name="yearcontent" onchange="document.myform.submit();">
<?php
		if($yearcontent!="") {
		 echo "<option value=$yearcontent>$lg_session $yearcontent</option>";
		}
		else {
		echo "<option value=''>$lg_year_session</option>";
		}
			$sql="select * from ses_stu where stu_uid='$uid' and sch_id='$sid' and year!='$yearcontent' order by year desc";
            $res=mysql_query($sql)or die("query failed:".mysql_error());
            while($row=mysql_fetch_assoc($res)){
                        $y=$row['year'];
...
<?php
		$read=mysql_query("SELECT * FROM ses_stu WHERE stu_uid='$uid' and year='$yearcontent'") or die(mysql_error());
		$select=mysql_fetch_array($read);
		$cls_code=$select['cls_level'];
		//echo "$uid $cls_code";
		$sql="select count(*) from content where app='NEWS'  and cat='ewaris' and  sta=1 and ( cls_code='$cls_code' or cls_code=0) and (sid=0 or sid='$xsid') and (access like '%IBUBAPA%' or access like '%PARENT%') $yearcont $sqlsort $sqlmaxline";
		...
		$sql="select content.* from content where app='NEWS'  and cat='ewaris' and  sta=1 and (sid=0 or sid='$xsid') and (cls_code='$cls_code' or cls_code=0) and (access like '%IBUBAPA%' or access like '%PARENT%') $yearcont $sqlsort $sqlmaxline";	
        $res=mysql_query($sql)or die("query failed:".mysql_error());
        while($row=mysql_fetch_assoc($res)){
				$usr=$row['usr'];
