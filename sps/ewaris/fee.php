...
if (count($checkbox)>0) {
	...
	$direktori="../usr_images/ewaris/$gambar";
	if(!empty($lokasi))
	{
	...
	$sql="insert into feeonlinetrans (cdate,year,sch_id,stu_id,stu_uid,rm,stu_ic,bank,bankno,image,paydate)
	values(now(),'$year',$sid,$stu_id,'$uid',$total,'$stu_ic','$bank','$bankno','$gambar','$paydate')";
...
function process_form(operation){
		...
		if(document.myform.bankno.value==""){
			alert("Masukkan tanda bukti pembayaran");
	...
	<table width="100%" id="mytitle"> 
		...
		<td>
			<select name="bank">
				...
				<option value="K.Kredit">Kartu Kredit</option>
				<option value="Transfer">Transfer</option>
				<option value="DLL">Pembayaran Lain</option>
			</select> <input name="nocek" type="text" id="nocek" size="20" placeholder="keterangan">
		(hanya untuk Pembayaran Lain) 
		</td>
		</tr>
		<tr>
		...
		<tr>
			<td>&nbsp;</td>
			<td><input type="file" name="gambar" id="gambar" value=50"> (Pilh gambar: JPG, PNG atau GIF)</td>
		</tr>
		<tr>
