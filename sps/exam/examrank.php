...
<div id="content">
  <div id="mypanel">
	...
	<div align="right" id="mytabletitle" class="printhidden">
...
<?php
		if($year=="") {
			echo "<option value=''>Pilih T.A</option>";
		} else {
			echo "<option value='$year'>T.A $year</option>";
		}
            ...
            while($row=mysql_fetch_assoc($res)){
                        $s=$row['prm'];
						$v=$row['val'];
                        echo "<option value=\"$s\">T.A $s</option>";
            }				  
?>
      </select>
...
<table width="100%" cellspacing="0" cellpadding="2">
	<tr>
              ...
              <td id="mytabletitle" width="10%" align="center"><?php echo strtoupper("$lg_matric");?></td>
			  <td id="mytabletitle" width="5%" align="center"><?php echo strtoupper("$lg_mf");?></td>
			  <td id="mytabletitle" width="32%">&nbsp;<?php echo strtoupper("$lg_name");?></td>
			  <td id="mytabletitle" width="10%">&nbsp;<?php echo strtoupper("$lg_class");?></td>
			  <td id="mytabletitle" width="20%" align="center">Jumlah Nilai</td>
			  <td id="mytabletitle" width="20%" align="center">Nilai Rata-rata</td>
	</tr>
	<?php	
...
?>
		...
		<?php
			$cek=mysql_query("select * from exam where stu_uid='$uid' and cls_code='$clscode' and year='$year' and examtype='$examcode'") or die (mysql_error());
			$total=0;
			while ($read=mysql_fetch_array($cek)){
				$point=$read['point'];
				$total=$total+$point;
			}
		?>
		...
		<?php
			$cek=mysql_query("select count(*) from exam where stu_uid='$uid' and cls_code='$clscode' and year='$year' and examtype='$examcode'") or die (mysql_error());
			$hitung=mysql_fetch_row($cek);
			$jummapel=$hitung[0];
			$rata=$total/$jummapel;
		?>
		<td id="myborder" align="center">&nbsp;<?php printf("%.02f",$rata);?> </td>
        </tr>    
<?php  
} ?> 	
