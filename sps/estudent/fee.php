...
if (count($checkbox)>0) {
	...
	if($bank=="CC")
			echo "<script language=\"javascript\">location.href='https://www.onlinepayment.com.my/NBepay/pay/test4957/?amount=$total&orderid=$feeid'</script>";
	else
...
function process_online(operation){
	...
	if((document.myform.bankno.value=="")&&(document.myform.bank.value!=1)){
			alert("Sila masukkan maklumat rujukan");
			document.myform.bankno.focus();
			return;
	}
		
	ret = confirm("Teruskan pembayaran?");
	if (ret == true){
...
<form name="myform" method="post" action="feeonline.php">
		<input type="hidden" name="p" value="fee">
<div id="panelleft" style="display:none;">
		 ...
		 <?php
			...
			if($found>0){
				...
				if($feeval==0){
					...
					$sql="select feeonlinepay.*,feeonlinetrans.sta from feeonlinepay inner join feeonlinetrans where tid=feeonlinetrans.id and feeonlinepay.stu_uid='$uid' and feeonlinepay.fee='$feename' and sta=0";
					$res2=mysql_query($sql) or die(mysql_error());
					$found=mysql_num_rows($res2);
					if($found)
						echo "- PENDING -";
...
?>
	<table width="100%" id="mytitle" style="font-size:12px"> 
       ...
       <tr>
		...
		<td>
         	<select name="bank" onChange="if(document.myform.bank.value==1){ show('onlinepayment');hide('otherpayment');}else{show('otherpayment');hide('onlinepayment');}">
                <option value="1">Kredit Kad</option>
				<option value="2">Bank Transfer</option>
                <option value="3">Tunai (Cash-Deposit)</option>
				<option value="4">Cek (Cheque-deposit)</option>
			</select>
		</td>
		</tr>
</table>
...
<?php } ?>
	<table width="100%" id="mytitle" style="font-size:12px">         
		<tr>
			<td width="20%" align="right">Tarikh&nbsp;Pembayaran</td>
            <td>
       ...
       <tr>
            <td align="right"></td>
            <td><input type="button" name="btnpayment" value="TERUSKAN BAYARAN" onClick="return process_online('')"></td>
       </tr>  
</table>
</div>
...
<table width="100%" id="mytitle" style="font-size:12px">         
       <tr>
            ...
            <td >
            	<input type="button" name="btnpayment" value="TERUSKAN BAYARAN" onClick="return process_online('')">
                <br>
				Service pembayaran ini adalah selamat/secured dan diuruskan oleh <br>
				Payment Gateway <a href="http://molpay.com" target="_blank">MOLPAY</a> dengan lebih 20 institusi  kewangan seluruh negara.)
            </td>
       </tr>  
</table>
