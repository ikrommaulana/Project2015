...
$username = $_SESSION['username'];
	...
	if($operation=="delete"){
			...
			if($id!=""){
					$sql="update parent set q0='$q0',q1='$q1',q2='$q2',q3='$q3',q4='$q4',q5='$q5',q6='$q6',q7='$q7',q8='$q8',q9='$q9',
					p1name='$p1name',p1ic='$p1ic',p1rel='$p1rel',p1job='$p1job',p1wn='$p1wn',p1edu='$p1edu',
					...
					p2name='$p2name',p2ic='$p2ic',p2rel='$p2rel',p2job='$p2job',p2wn='$p2wn',p2edu='$p2edu',
					...
					adm='$username',ts=now() where id=$id";
					mysql_query($sql)or die("$sql query failed:".mysql_error());
					$f="<font color=blue>&lt;SUCCESSFULLY UPDATED&gt</font>";
			}else{	
					$OKTOSAVE=1;
					$sql="select p1ic from parent where p1ic='$p1ic' and sid=$sid";
					$res=mysql_query($sql)or die("query failed:".mysql_error());
					$num=mysql_num_rows($res);
					if($num>0){
							$f="<font color=red size=3>&lt;Pendaftaran Gagal. Kad pengenalan '$p1ic' telah wujud&gt</font>";
							$OKTOSAVE=0;
					}
					$sql="select acc from parent where acc='$acc' and sid=$sid";
					$res=mysql_query($sql)or die("query failed:".mysql_error());
					$num=mysql_num_rows($res);
					if($num>0){
							$f="<font color=red size=3>&lt;Pendaftaran Gagal. Nombor akaun '$acc' telah wujud&gt</font>";
							$OKTOSAVE=0;
					}
					
					if($OKTOSAVE){
							$sql="insert into parent(dtm,sid,acc,q0,q1,q2,q3,q4,q5,q6,q7,q8,q9,
							p1name,p1ic,p1rel,p1job,p1sal,p1com,p1addr,p1state,p1tel,p1tel2,p1fax,p1hp,p1mel,p1bandar,p1poskod,p1wn,p1edu,
							p2name,p2ic,p2rel,p2job,p2sal,p2com,p2addr,p2state,p2tel,p2tel2,p2fax,p2hp,p2mel,p2bandar,p2poskod,p2wn,p2edu,
							p3name,p3ic,p3rel,p3addr,p3tel,p3tel2,p3fax,p3hp,p3mel,
							adm,ts) values
							(now(),$sid,'$acc','$q0','$q1','$q2','$q3','$q4','$q5','$q6','$q7','$q8','$q9',
							'$p1name','$p1ic','$p1rel','$p1job','$p1sal','$p1com','$p1addr','$p1state','$p1tel','$p1tel2','$p1fax','$p1hp','$p1mel',
							'$p1bandar','$p1poskod','$p1wn','$p1edu',
							'$p2name','$p2ic','$p2rel','$p2job','$p2sal','$p2com','$p2addr','$p2state','$p2tel','$p2tel2','$p2fax','$p2hp','$p2mel',
							'$p2bandar','$p2poskod','$p2wn','$p2edu',
							'$p3name','$p3ic','$p3rel','$p3addr','$p3tel','$p3tel2','$p3fax','$p3hp','$p3mel',
							'$username',now())";
							mysql_query($sql)or die("$sql query failed:".mysql_error());
							$f="<font color=blue size=3>&lt;SUCCESSFULLY REGISTER&gt</font>";
					}
			}
	}
