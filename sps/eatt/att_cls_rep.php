...
include_once("$MYLIB/inc/language_$LG.php");
	...
	$mn=$rowmonth['name'];
	/*
	if ($mn == 'January') {$mn="Januari";}
	else if ($mn == 'February') {$mn="Februari";}
	else if ($mn == 'March') {$mn="Maret";}
	else if ($mn == 'April') {$mn="April";}
	else if ($mn == 'May') {$mn="Mei";}
    else if ($mn == 'Jun') {$mn="Juni";}
	else if ($mn == 'July') {$mn="Juli";}
	else if ($mn == 'August') {$mn="Augustus";}
	else if ($mn == 'September') {$mn="September";}
	else if ($mn == 'October') {$mn="Oktober";}
	else if ($mn == 'November') {$mn="November";}
	else if ($mn == 'December') {$mn="Desember";}
	else {$mn=$mn;}
	*/
...
<title>KEHADIRAN <?php echo strtoupper($clsname);?></title>
...
<?php if($LG=="BM"){?>
	Tips&raquo;<br>
	1. Pilih Sekolah dan pilih kelas terlebih dahulu.<br>
	2. Klik pada TANGGAL untuk memasukkan kedatangan.<br>
	3. Klik pada NAMA pelajar untuk melihat laporan tahunan pelajar.<br>
	4. Optional. Silahkan print jadwal kosong mengikuti bulan dan tandai secara manual dahulu.<br>
	5. *Jadwal absensi sekolah perlu di siapkan dahulu untuk mengguna servis ini.<br>
<?php }else{?>
	Tips&raquo;<br>
...
<table width="100%" border="0" id="mytitle">
	  <tr>
			<td width="5%"><a href="#" onClick="process_change('-1')"><strong>BULAN SEBELUMNYA</strong></a></td>
			<td align="center"><?php echo strtoupper(" $mn $y");?></td>
			<td width="5%" align="right"><a href="#" onClick="process_change('1')"><strong>BULAN SELANJUTNYA</strong></a></td>
	  </tr>
</table>
...
<?php
		for($i=1;$i<=$nd;$i++){
			...
			$ds=date('N',mktime(0,0,0,$mm,$i,$y)); // get day 1-7
			switch($ds){
				case 1 :
					$dn="Senin";
				break;
...
?>
  <tr bgcolor="<?php echo $bg;?>" onMouseOver="this.bgColor='#CCCCFF';" onMouseOut="this.bgColor='<?php echo $bg?>';">
    ...
    <td id="myborder" style="font-size:95%"><a href="../eatt/att_stu_rep.php?uid=<?php echo "$uid&sid=$sid&year=$year"?>" title="<?php echo $lg_student_record;?>" onClick="return GB_showPage('<?php echo $lg_attendance;?>',this.href)" target="_blank"><?php echo $name?></a></td>
<?php
				...
				if($sta=="") {
					...
					if($des!="")
						...
						elseif($des=="A"){
							...
							$xdes="<a href=\"#\" title=\"$ket\"><font color=#FFFFFF><strong>$des</strong></font></a>";
				}
				else 
		...
		//kira jum tak hadir
			...
			$sql="select count(*) from stuatt where stu_uid='$uid' and sta=0 and d like '$thy%' and year='$year'";
			$res2=mysql_query($sql)or die("query failed:".mysql_error());
			$row2=mysql_fetch_row($res2);
			$thyear=$row2[0];
			
			$sql="select count(*) from stuatt where stu_uid='$uid' and sta=0 and d like '$thm%' and year='$year'";
			$res2=mysql_query($sql)or die("query failed:".mysql_error());
			$row2=mysql_fetch_row($res2);
			$thmonth=$row2[0];
?>
