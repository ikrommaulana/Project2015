...
<tr>
<?php
	$sql="select distinct(level) from cls where sch_id='$sid' order by level";
...
<?php 
$chart_note_center=$lg_total. " A";
$chart_item_value="";
$chart_item_group="";
$chart_note_top="$namatahap $clslevel";

		//$sql="select count(*) from sub where sch_id='$sid' and level='$clslevel' and grptype=0 and credit>0";
		$sql="select max(g1) from examrank where sch_id='$sid' and cls_level='$clslevel' and year=$year and exam='$exam'";
    	$res=mysql_query($sql)or die("query failed:".mysql_error());
		...
		for($i=$jumlah_max_a;$i>=0;$i--){
				$tmp=$i."A";
				if($chart_item_group!="")
						$chart_item_group=$chart_item_group.",";
				$chart_item_group=$chart_item_group.$tmp;
				
				$sql="select count(*) from examrank	 where sch_id=$sid and year='$year' and cls_level=$clslevel and exam='$exam' and g1='$i'";
