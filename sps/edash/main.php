...
<?php 
$chart_note_top="Siswa";
$chart_note_left="$lg_total $lg_student $year";
$chart_note_center="";
//$chart_item_group="Rendah,Menengah,Tahfiz,Tadika";
//$chart_item_value="800,900,1000,1100";
$chart_item_value="$lg_student";
$chart_item_group="";
$sql="select * from sch where id>0 order by id";
...
while($row=mysql_fetch_assoc($res)){
		...
		$sql="select count(*) from stu where status=6 and sch_id=$xsid";
...
<?php 
$chart_note_top="";
$chart_note_left="$lg_total_student $year";
$chart_note_center="";
//$chart_item_group="Rendah,Menengah,Tahfiz,Tadika";
//$chart_item_value="800,900,1000,1100";
...
$sql="select id,sname from sch where id>0 order by id";
$res=mysql_query($sql)or die("query failed:".mysql_error());
while($row=mysql_fetch_assoc($res)){
		$s=$row['sname'];
		$x=$row['id'];
		...
		$sql="select count(*) from stu where sch_id='$x' and status=6 and sex=1";
		$res2=mysql_query($sql)or die("query failed:".mysql_error());
		$row2=mysql_fetch_row($res2);
		$t=$row2[0];
		if($totalstu!="")
		...
		$sql="select count(*) from stu where sch_id='$x' and status=6 and sex=0";
		$res2=mysql_query($sql)or die("query failed:".mysql_error());
		$row2=mysql_fetch_row($res2);
		$t=$row2[0];
...
for($i=1;$i<=6;$i++){
	if($i==1)
		$chart_item_value="Level 1";
	else
		$chart_item_value=$chart_item_value.";Level $i";
		
	$sql="select id,sname from sch where id>0 order by id";
	$res=mysql_query($sql)or die("query failed:".mysql_error());
	while($row=mysql_fetch_assoc($res)){
			$x=$row['id'];
			$sql="select count(*) from stu INNER JOIN ses_stu ON stu.uid=ses_stu.stu_uid where ses_stu.sch_id=$x and ((stu.status=6)) and ses_stu.cls_level=$i and year='$year'";
			$res2=mysql_query($sql)or die("query failed:".mysql_error());
			$row2=mysql_fetch_row($res2);
			$t=$row2[0];
			//echo "$i=$x:$t - ";
...
<?php 
$chart_note_top="";
$chart_note_left="$lg_total $lg_teacher";
$chart_note_center="";
//$chart_item_group="Rendah,Menengah,Tahfiz,Tadika";
//$chart_item_value="800,900,1000,1100";
$chart_item_value="$lg_teacher";
$chart_item_group="";
$sql="select * from sch where id>0 order by id";
$res=mysql_query($sql)or die("query failed:".mysql_error());
while($row=mysql_fetch_assoc($res)){
...
<?php 
$chart_note_top="";
$chart_note_left="$lg_teacher $year";
$chart_note_center="";
//$chart_item_group="Rendah,Menengah,Tahfiz,Tadika";
//$chart_item_value="800,900,1000,1100";
