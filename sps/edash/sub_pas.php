    ...
    <td width="50%" valign="top">
		<table width="100%"  border="0" cellpadding="0">
		  ...
		  <tr>
			...
			<td>:</td>
			<td><?php echo strtoupper($lg_pass_percentage);?>
			</td>
		  </tr>
		</table>
...
$chart_item_value="";
$chart_item_group="";
$sql="select distinct(sub_code) from ses_sub where sch_id=$sid and year='$year' and cls_level='$clslevel' $sqlcons order by sub_name";
$res=mysql_query($sql)or die("$sql:query failed:".mysql_error());
while($row=mysql_fetch_assoc($res)){
			$sc=$row['sub_code'];
			...
			$chart_note_left="$namatahap $clslevel";
			$gp=0;$totalgp=0;$totalstu=0;$total_fail=0;$total_pass=0;
			...
			while($row2=mysql_fetch_assoc($res2)){
					...
					$gp=$row2['gp'];
					$isfail=$row2['sta'];
					$sql="select count(*) from exam where sch_id=$sid and year='$year' and cls_level=$clslevel $sqlclscode and sub_code='$sc' and examtype='$exam' and grade='$p' and gp>=0 and (grade!='TT' and grade!='TH')";
			...
			if($totalstu>0){
					$xx=round($total_pass/$totalstu*100,1);
					$yy=$xx-100;
					$strpass=$strpass.",". $xx;
					$strfail=$strfail.",". $yy;
					//echo "$sc:$total_pass/$totalstu<br>";
			}else{
				$strpass=$strpass.","."0";
				$strfail=$strfail.","."0";
			}
...
$chart_item_value="$strpass;$strfail";
$xml="chart_stack1.php?dat=$chart_item_group|$chart_item_value|$chart_note_left|$chart_note_top|$chart_note_bottom|$chart_note_center|$chart_note_right|$chart_decimal_value";
//echo "$xml<br><br>";
